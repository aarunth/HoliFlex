                        ...
                        for link in dd_monthly.get('links', []):
                            if link.get('rel') == 'standard_unit_rates':
                                ...
                                if rates_response.status_code == 200:
                                    ...
                                    # Sort results by valid_from to get latest rates
                                    if 'results' in rates_data:
                                        rates_data['results'].sort(key=lambda x: x.get('valid_from', ''), reverse=True)
                                    return rates_data
                ...
                if rates_info:
                    ...
                    if rates_info:
                        ...
                        for rate in latest_rates:
                            print(f"  Rate: {rate['rate']:.2f}p/kWh ({rate['from'].strftime('%H:%M')}-{rate['to'].strftime('%H:%M')})")
                else:
                        ...
                        if tariffs:
                            ...
                            if 'direct_debit_monthly' in tariff_details:
                                ...
                                if 'standard_unit_rate_inc_vat' in dd_monthly:
                                    ...
                                    print(f"Standard Unit Rate: {rate:.2f}p/kWh (Valid from: {now.strftime('%Y-%m-%d %H:%M')})")
                                    continue
