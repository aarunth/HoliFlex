...
def main():
    try:
        ...
        if products:
            ...
            agile_products = [p for p in products if 'AGILE' in p.get('code', '').upper()]
            
            if not agile_products:
                print("No Agile tariffs found.")
                return
            
            print("\nAgile Octopus Tariffs:")
            print("=" * 50)
            
            for product in agile_products:
                rates = fetch_rates(api_key, product['code'])
                if rates and 'results' in rates:
                    results = rates['results']
                    if len(results) > 1:  # Only show if there are multiple rates
                        # Process rates
                        rates_info = []
                        for rate in results:
                            try:
                        ...
                        if len(rates_info) >= 2:  # Only show if we have at least 2 valid rates
                            # Sort by rate value
                            rates_info.sort(key=lambda x: x['rate'])
                            
                            # Print concise product summary with min/max rates
